...
export type MenuType = 'character' | 'province' | 'region' | 'army' | 'trade' | 'diplomacy' | 'governance' | 'state_owner' | 'none'
...
export type MapMode = 'terrain' | 'population' | 'settlement' | 'wealth' | 'culture' | 'governance' | 'sovereignty' | 'rivers' | 'trade' | 'food'
...
export interface TradeFlow {
  ...
  value: number                // gross value (volume × price), pre-decay
  effective_value?: number     // value × efficiency — actual income distributed
  efficiency?: number          // 0.0–1.0 proximity efficiency (distance penalty)
  transit_cluster_ids?: string[] // intermediate clusters earning waypoint income
